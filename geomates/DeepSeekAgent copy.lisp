...
;; Production to handle case when no diamond is found
(p diamond-search-failed
   =goal>
      state searching-for-diamond
   =imaginal>
      current-goal searching
...
==>
   =goal>
      state try-search-method-2
   =imaginal>
      current-goal broader-search
   +visual-location>
      isa polygon-feature
   !output! (Direct diamond search failed trying broader search)
)

;; Production to try a different search method
(p try-different-search-method
   =goal>
      state try-search-method-2
   =imaginal>
      current-goal broader-search
...
==>
   ...
   !output! (Broader search failed falling back to random exploration)
)

;; Production to find diamond with broad search
(p found-any-object
   =goal>
      state try-search-method-2
   =imaginal>
   ...
   ?visual>
      state free
==>
   !eval! (if (string= =val "diamond")
             (mod-buffer-chunk 'goal '(state found-diamond))
             (mod-buffer-chunk 'goal '(state check-next-object)))
   =imaginal>
   +visual>
      cmd move-attention
      screen-pos =visual-location
   !output! (Found object with value =val)
)

;; Production to check next object if current one isn't a diamond
(p check-next-object
   =goal>
      state check-next-object
   =imaginal>
      current-goal broader-search
   ?visual-location>
      state free
==>
   ...
   +visual-location>
      :nearest current
      :attended nil
   !output! (Checking next visual object)
)
...
==>
   ...
   !output! (Found diamond at x =x y =y - planning path)
)
...
==>
   ...
   !output! (Rectangle player planning horizontal movement first)
)

;; Production to plan approach for disc player
(p plan-disc-approach
   =goal>
      state planning-path-to-diamond
   =imaginal>
      isa player-state
...
==>
   ...
   +visual-location>
      isa polygon-feature
      value "diamond"
   !output! (Reassessing failed searching for diamond again)
)
